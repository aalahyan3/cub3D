...
void	draw_strip(int x, t_all_data *data, t_proj pr)
{
	...
	while (y < pr.draw_e)
	{
		...
		p = data->tex_data.tex->addr
			+ data->tex_data.tex_y * data->tex_data.tex->line_length
...
void	draw_wall(int x, t_proj pr, t_all_data *data)
{
	...
	data->tex_data.tex_x = (int)((data->tex_data.x_offset / TILE_SIZE)
			* data->tex_data.tex->width);
...
void	the_3d_projection(int i, t_player *p, t_all_data *data)
{
	t_proj	pr;
	double	proj_p;
	int		x;
	...
	pr.corr_dist = data->rays->rays_dis * cos(data->rays->ray_angl - p->pa);
...
void	start_casting(t_player *player,
		t_img *img, t_map *map, t_all_data *data)
{
	t_rays	rays;
	int		i;
	...
	while (i < player->num_rays)
	{
		...
		the_3d_projection(i, player, data);
		i++;
	}
}
